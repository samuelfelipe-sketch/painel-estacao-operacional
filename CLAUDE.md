...
- **Tela de notificações**: o sino na topbar/desktop dos painéis abre um painel lateral no PE (`window.abreNotifs`, âncora `estrategia/#notificacoes`) com o histórico de follow-ups (sem apagados, mais recentes primeiro), destaque laranja + contador de não lidos (localStorage `sapatao-notif-vistos-v1`; follow-up do próprio usuário já nasce lido; clicar num item abre o cartão da ação) e link para ativar o push do aparelho. No Guia o sino leva a `../estrategia/#notificacoes`; o toque na notificação push também abre essa tela.
- **Push nativo (Web App)**: `sw.js` + `manifest.json` na raiz; cartão "Notificações no celular" nas Configurações centrais inscreve o aparelho (Push API) e grava a inscrição CIFRADA (AES-GCM + RSA-OAEP) em `roadmap/push-subs.json` (`{u:id,em,env}`). Eventos que notificam (via mensagem do commit automático): follow-up novo no PE (`novo follow-up de X na ação N`) e mudança de estado nos DOIS painéis (`X concluiu/reabriu a ação N do Roadmap Comercial|Planejamento Estratégico`). O workflow `.github/workflows/notifica-push.yml` (paths: `pe-execucoes.json` e `execucoes.json`) roda `scripts/notifica-push.mjs`, que decifra as inscrições, **pula os aparelhos do próprio autor** (nome do commit → id via `usuarios.json`) e envia via Web Push **sem título** (o iOS mostra só o nome do app "Sapatão"; o painel vai no texto do aviso); toque abre o painel certo. Cada inscrição pode ter `pref:{fu,est}` (caixinhas no cartão das Configurações) — o robô pula aparelhos que desligaram aquele tipo de aviso. Segredos do repositório: `VAPID_PRIVATE_KEY` e `PUSH_SUBS_PRIVATE_KEY` (chaves privadas; as públicas estão no código). No iPhone só funciona com o site adicionado à Tela de Início. Conteúdo do follow-up NUNCA sai do cofre — o aviso é só autor + nº da ação.
