...
- **Lembrete diário de prazos**: os painéis publicam `roadmap/calendario.json` (SÓ nº da ação + prazo + status, sem conteúdo — decisão aprovada pelo Samuel) junto do sync; o workflow `lembra-prazos.yml` (cron 12:00 UTC = 9h BRT) roda `scripts/lembra-prazos.mjs` e avisa "vence hoje/amanhã", "vence em 7 dias" (só no dia exato), "atrasou (venceu ontem)" e pega carona nas já atrasadas — tipo `przo`.
