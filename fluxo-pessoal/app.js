...
function renderMetas(){
  ...
  const cfg = metasCfg();
  // custo essencial mensal: soma das medianas mensais por categoria, sem as
  // categorias não essenciais (aplicações, casamento, viagens, esportes,
  // casa/móveis). A mediana de cada categoria ainda ignora gastos pontuais
  // (ITBI/cartório, repasses únicos) sem descartar a parte recorrente.
  const NAO_ESSENCIAL = ['aplic','casamento','viagens','esportes','casa_mov'];
  let despRef = 0;
  if (FECHADOS > 0)
  ...
  let h = `<div class="kv tt"><span class="k">Reserva de emergência<small>só o que resgata em até D+30 · custo essencial (medianas por categoria em ${ANO} — sem aplicações, casamento, viagens, esportes, casa/móveis e gastos pontuais como ITBI): ${fmt(-despRef)}/mês</small></span><span class="v">${mesesCob.toFixed(1)} meses</span></div>
